...
- `hashing()` ― 全件を素の AoH（ハッシュリファレンスのリスト）で。**非推奨・レガシー対応専用**。新規コードは `hashes()` を使う
...
`hashing()` は **非推奨（レガシー対応専用）**の取得形式である。metaAoh ではなく素の AoH を直接欲しいレガシー資産のために残すが、新規コードは `hashes()` を使う。実装は `hashes()->toAoh()`（0件なら空リスト）で、undef→`''` 変換など値の扱いは `hashes()` と同じ。レガシー専用のため効率最適化はせず metaAoh 構築を経由する。`廃止予定` ではない（将来の廃止は別途検討する）。
...
`spool($spool_id, @confirm)` は実行済みステートメントハンドルから 1 行ずつ `Spool->open` / `add` / `close` へ流し、続けて Spool の確定（confirm）まで行って `$spool_id` を返す。確定モードは `@confirm` の形で判別し、Spool の confirm API の引数の形と1対1に対応する。空なら `line`（行単位・順序不問）、文字列（カラム名）の並びなら `records`（連続同一キーのグループ化）、配列リファレンスの並びなら `grouping`（階層グループ）。
...
- `@confirm`: spool の確定モード指定。空 = `line`、文字列（カラム名）の並び = `records`、配列リファレンスの並び = `grouping`
...
| `hashing()` | 全件（素の AoH）**非推奨・レガシー対応** | ― | ハッシュリファレンスのリスト（0件で空リスト） |
...
# データ取得は get(), list(), arrays(), hashes() に限定する。hashing() は非推奨・レガシー対応専用の追加形式とする（新規は hashes() を使う）
...
# DB から取得した値に undef が含まれていた場合は、get(), list(), arrays(), hashes() の返却時に '' へ置き換える（hashing() は hashes() を経由するため同じ規則が適用される）
...
# hashing() は非推奨・レガシー対応専用の取得形式である（新規コードは hashes() を使う。廃止予定ではない）。hashes() の metaAoh を平坦化した素の AoH（ハッシュリファレンスのリスト）を返す。実装は hashes()->toAoh()。0件なら空リスト。hashing は hashes の薄い変換であり独自の取得処理は持たない。レガシー専用のため効率最適化はせず metaAoh 構築を経由する
...
# spool() の確定モードは @confirm の形で判別する。空なら line、文字列（カラム名）の並びなら records、配列リファレンスの並びなら grouping。判別は先頭要素の形のみで行う
