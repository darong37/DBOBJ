...
- データ取得は `get()`、`list()`、`arrays()`、`hashes()` の4形式に限定する。`hashing()` は非推奨・レガシー対応専用の追加形式（新規コードは `hashes()` を使う）
...
### hashing()（非推奨・レガシー対応専用）

- **非推奨**。metaAoh ではなく素の AoH（ハッシュリファレンスのリスト）を直接欲しいレガシー資産のために残す。新規コードは `hashes()` を使う。`廃止予定` ではない（将来の廃止は別途検討）
- `hashes()` の metaAoh を平坦化した素の AoH を返す。実装は `hashes()->toAoh()`、0件なら空リスト
- `undef`→`''` 変換・カラム情報の扱いは `hashes()` に従う（独自の取得処理を持たない薄い変換）
- レガシー専用のため効率最適化はせず、metaAoh を構築してから平坦化する経路のままとする
