...
| `spool($spool_id)` | 全行を Spool へ退避して spool_id を返す |
...
# Spool（巨大な結果セットを 1 行ずつディスクへ退避）
$db->run("SELECT dept, id FROM users ORDER BY dept, id");
$db->spool('job001');               # 'job001' を返す
my $count = Spool::records('job001', @{$db->{ordercols}});  # 確定は呼び出し側が行う
...
- `spool()` は結果セットをメモリに作らずに 1 行ずつ流す。トップレベルの `ORDER BY` が必須で、
  各指定が裸の列名に解決できること（位置指定・式・修飾名・引用識別子は die）。
  解析したソート列は `$db->{ordercols}` に保持される。DBOBJ は SQL を書き換えない
